...
if __name__ == '__main__':
	...
	for filename in args.files:
		...
		entities={
			...
			"Slice_XZ_extent_theta":str(extents[1]*2),
		...
		if args.aux and not (args.disable and "abundance" in args.disable):
			...
			if num_cores!=1:
				...
				for sl in range(0,n_hyperslabs):
					E_RMS_array[:,sl*step:(sl+1)*step,:]=compute_E_RMS_XY_wedge(sl)
			...
			# Corresponding xdmf for z slice (XY plane):
			extents_str="&Slice_"+con_sp[S_P]+"_extent_theta; &Extent_r;"
			dims_str=str(dims[2])+" &Dim_r;"
			...
			for species in range(0,n_species):
				...
				lumin_array[species] = np.vstack(Parallel(n_jobs=num_cores)(delayed(compute_luminosity_z)(0,sl) for sl in range(0,n_hyperslabs)))
			qprint("Done with "+S_P+" slice.\nWriting results:")
			for n,sp in enumerate(['e','e-bar','mt','mt-bar']):
				qprint("	Writing /radiation/Luminosity_"+sp+" out to "+con_sp[S_P]+" HDF5 file")
				reduced_hf[S_P].create_dataset("/radiation/Luminosity_"+sp,data=lumin_array[n])
			for n in ['e','e-bar','mt','mt-bar']:
				qprint("	Creating "+con_sp[S_P]+"/radiation/Luminosity_"+str(n)+" xdmf element")
				radiation_xdmf('Luminosity_'+n,'Luminosity_'+n,S_P,'/Radiation','/radiation',dims_str,extents_str)
			# qprint("########################################")
			# for n,lumin in enumerate(lumin_array):
			# 	qprint("Writing luminosity species "+str(n)+" to auxilary hdf file")
			# 	reduced_hf.create_dataset("/radiation/Luminosity_"+str(n),da`ta=lumin)
