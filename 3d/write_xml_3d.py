...
if __name__ == '__main__':
	...
	for filename in args.files:
		...
		if args.aux:
			qprint("--- Elapased time: %s seconds ---" % (time.time()-old_time))
			qprint("Creating derived values")
			qprint("Reading in radiation data from HDF5 files")
			...
			qprint("Done --- Total elapsed time: %s seconds ---" % (time.time()-old_time))
			qprint("Creating E_RMS_values:")
			...
			qprint("--- Total elapsed time: %s seconds ---" % (time.time()-old_time))
			qprint("########################################")
