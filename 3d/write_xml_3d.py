...
try:
	...
	dimstr = str(dims[2]/hf['mesh']['nz_hyperslabs'].value)+" "+str(dims[1])+" "+str(dims[0])#dimstr=""
	# create xdmf element
	...
	for name in storage_names:
		at = et.SubElement(grid['Hydro'],"Attribute",Name=name,AttributeType="Scalar",Center="Cell",Dimensions=str(dims[2])+" "+str(dims[1])+" "+str(dims[0]))
		# col=et.SubElement(at,"DataItem",ItemType="Collection",NumberType="Float",Precision="8")
		for n in range(1, hf['mesh']['nz_hyperslabs'].value+1):
			et.SubElement(at,"DataItem",Dimensions=dimstr,NumberType="Float",Precision="8",Format="HDF").text= filename[:-4] + str(n) + ".h5:/fluid/" + storage_names[name]
